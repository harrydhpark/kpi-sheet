...
regions.forEach(region => {
  ...
  rowsList.forEach(row => {
    ...
    // For competitor table (rows 83-93, 시장판가 ASP & M/S), use y25_prev directly from KPI(26년) 2025 range
    if (row.index >= 83 && row.index <= 93 && row.y25_prev && row.y25_prev.some(v => v !== null && v !== 0)) {
...
function computeRatios(rowList, yearKey) {
  ...
  // G.ASP ($) rows: Gross Sales / Sell-in Qty * 1e6 (scaled for formatVal / 1e6)
  divRows(38, 19, 41, 1e6);
  divRows(39, 22, 42, 1e6);
  divRows(40, 23, 43, 1e6);
  
  // N.ASP ($) rows: Net Sales / Sell-in Qty * 1e6 (scaled for formatVal / 1e6)
  divRows(47, 19, 53, 1e6);
  divRows(48, 22, 54, 1e6);
  divRows(49, 23, 55, 1e6);
...
<body class="flex min-h-screen">
    ...
    <main class="ml-72 flex-1 flex flex-col min-h-screen min-w-0">
        ...
        <header class="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-12 sticky top-0 z-40">
            <div class="flex items-center gap-6">
                ...
                <p class="text-sm font-semibold text-primary" id="page-indicator">26년 7월 실적 + 8월차 이동</p>
            </div>
            <div class="flex items-center gap-6">
                <!-- Year Selector Tabs -->
    ...
    <script>
        ...
        function updateView() {
            ...
            document.getElementById('page-indicator').innerText = '26년 7월 실적 + 8월차 이동';
        ...
        function renderSummaryCards(rows) {
            ...
            const netSalesRow = rows.find(r => r.index === 50);
            const opIncomeRow = rows.find(r => r.index === 59);
            ...
            cardRowDefs.forEach(def => {
                ...
                if (currentYear === 2026) {
                    // 2026: YTD 7월 누적 (or 7월 말)
                    periodLabel = isStockMetric ? 'YTD (7월 말)' : 'YTD (7월 누적)';
                    
                    if (isStockMetric) {
                        // 유통재고: 7월 (index 7)
                        val = row.y26 ? row.y26[7] : null;
                        prevVal = row.y25 ? row.y25[7] : null;
                        yoy = (val !== null && prevVal !== null && prevVal !== 0) ? ((val - prevVal) / Math.abs(prevVal)) * 100 : null;
                    } else if (def.index === 65) {
                        ...
                        const val26_Jul = row.y26 ? (row.y26[7] || 0) : 0;
                        val = val26_H1 + val26_Jul;
                        
                        const val25_H1 = row.y25 ? (row.y25[6] || 0) : 0;
                        const val25_Jul = row.y25 ? (row.y25[7] || 0) : 0;
                        prevVal = val25_H1 + val25_Jul;
                        
                        yoy = (prevVal !== 0) ? ((val - prevVal) / Math.abs(prevVal)) * 100 : null;
                    }
                } else {
                    // 2025: 연간 실적 (TTL / 12월 말) & 2024년 연간 대비
                    periodLabel = '연간 실적';
                    
                    if (isStockMetric) {
                        // 유통재고: 12월 말 (index 12)
                        val = row.y25 ? row.y25[12] : null;
                        prevVal = row.y24 ? row.y24[12] : null;
                        yoy = (val !== null && prevVal !== null && prevVal !== 0) ? ((val - prevVal) / Math.abs(prevVal)) * 100 : null;
                    } else if (def.index === 65) {
                        ...
                        prevVal = row.y24 ? row.y24[17] : null;
                        yoy = (prevVal !== null && prevVal !== undefined && prevVal !== 0) ? ((val - prevVal) / Math.abs(prevVal)) * 100 : null;
                    }
                }
